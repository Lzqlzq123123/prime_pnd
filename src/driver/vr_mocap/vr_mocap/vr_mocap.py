...
for _group, limit in limit_config.items():
    for bone_id in limit.body_id:
        ip_specific_limit[bone_id] = limit


def calculate_velocity(previous_transform: Transform, current_transform: Transform, dt: float):
    distance = (
    ...
    rotation_previous = Rotation.from_quat(
        [
    ...
    rotation_current = Rotation.from_quat(
        [
...
class VRMocap(Node):
    def __init__(self):
        ...
        self._robot_protect_client = self.create_client(Trigger, "/adam_retarget/warm_start")
    ...
    def receive_data(self):
        while rclpy.ok():
            data, _addr = self.sock.recvfrom(1024 * 20)
            time_stamp = self.get_clock().now()
            # self.get_logger().info(f"Received message: {data.decode()} from {_addr}")
            ...
            for idx_s, pose in recv_j.items():
                ...
                if not protect_flag and idx in ip_specific_limit:
                    ...
                    dt = (ros_now.sec - ros_prev.sec) + (ros_now.nanosec - ros_prev.nanosec) * 1e-9
                    lin_vel, ang_vel = calculate_velocity(self._data[idx].transform, new_frame, dt)
                    if (
