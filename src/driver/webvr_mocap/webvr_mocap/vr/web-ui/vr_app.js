...
AFRAME.registerComponent('controller-updater', {
  ...
  initWebSocket: function () {
    ...
    const websocketUrl = `wss://${serverHostname}/vrwebsocket`;
    ...
    try {
      ...
      this.websocket.onclose = (event) => {
        ...
        // 检查是否是因为连接限制被拒绝
        if (event.code === 1008 && event.reason && event.reason.includes('Only one VR headset')) {
          console.warn('连接被拒绝：服务器已有一个VR头显连接');
          // 不显示alert，因为onmessage中已经处理了
      ...
      this.websocket.onmessage = (event) => {
        ...
        try {
          ...
          // 检查是否是服务器拒绝连接的消息
          if (data.error === 'VR_HEADSET_LIMIT') {
            console.error('连接被拒绝:', data.message);
  ...
  initState: function () {
    ...
    // this.rightHand.addEventListener('buttondown', (event) => { // Add gripup listener

    //   const id = event.detail.id;
    //   if (id == 5) {
    //     const videoA = document.querySelector('#robot-a-video');
    //     const isVisible = videoA.getAttribute('visible');
    //     videoA.setAttribute('visible', !isVisible);
    ...
    //     this.startVideoPos = videoA.getAttribute('position');
    //     this.startControllerPos = this.rightHand.object3D.position.clone();
    //   }
  ...
  tick: function () {
    ...
    //     videoA.setAttribute('position', newPos);
