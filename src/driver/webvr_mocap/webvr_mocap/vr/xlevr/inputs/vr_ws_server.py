...
class VRWebSocketServer(BaseInputProvider):
    ...
    def setup_ssl(self) -> ssl.SSLContext | None:
    ...
    async def start(self):
        ...
        try:
            ...
            # )
            self.server = await websockets.serve(self.websocket_handler, "127.0.0.1", port)
    ...
    async def websocket_handler(self, websocket, path=None):
        ...
        if len(self.clients) > 0:
            ...
            try:
                ...
                await websocket.close(code=1008, reason="Only one VR headset connection allowed")
            except Exception as e:
