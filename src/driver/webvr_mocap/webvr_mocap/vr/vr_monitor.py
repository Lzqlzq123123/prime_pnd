...
class VRMonitor:
    ...
    def __init__(self, vr_src_path: str | None = None):
        self.config = None
        self.vr_server = None
    ...
    def initialize(self):
        ...
        # Import modules
        (xlevr_config_cls, vr_websocket_server_cls, _, _) = import_xlevr_modules()
        if xlevr_config_cls is None:
            print("❌ Failed to import xlevr modules")
            return False

        # Create configuration
        self.config = xlevr_config_cls()
        ...
        try:
            self.vr_server = vr_websocket_server_cls(
    ...
    async def start_monitoring(self):
        ...
        try:
            ...
            host_display = (
                get_local_ip() if self.config.host_ip == "127.0.0.1" else self.config.host_ip
            )
    ...
    async def monitor_commands(self):
        ...
        while self.is_running:
            try:
                ...
                print("⚠️  No VR command received in the last 3 seconds, still waiting...")
                continue
            except Exception as e:
