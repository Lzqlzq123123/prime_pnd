...
def jetson_ssh_exec(host=JETSON_HOST, user=JETSON_USER, ssh_key=None, command=None, timeout=3):
    if ssh_key is None:
        ssh_key = get_ssh_key()
    ssh_cmd = ["ssh"]
    if ssh_key:
...
def start_synchronized_service(command):
    ...
    log_thread = threading.Thread(target=remote_logger, args=(proc.stdout,), daemon=True)
    log_thread.start()
