...
def download_package_and_transfer_to_jetson():
    ...
    with open(f"{SCRIPT_DIR}/common.py") as f:
        common_code = f.read()
    python_code = f"""
{common_code}
...
def setup_jetson():
    ...
    if jetson_package_version != "" and not compare_version(
        ...
        if jetson_network_connection:
            logger.info("开始在 Jetson 上部署...")
            jetson_handle()
        elif local_network_connection:
    ...
    start_synchronized_service(command=f"cd {HIDE_PATH}/{APP_NAME} && ./bin/pnd-webrtc-server")
