...
class WebVRMocap(Node):
    ...
    def _validate_goal(self, goal: dict | None) -> bool:
    ...
    def _process_and_publish_transforms(self, goal: dict) -> None:
        ...
        for data, bone in zip(self._data, self.goal_bones, strict=True):
            bone_data = goal[bone]
            ...
            data.child_frame_id = f"{bone}_uncalibrated" if not self.calibrated else bone
    ...
    def calibrate(self, goal: dict) -> None:
        ...
        self.robot_scale = (0.5 * right_arm_length + 0.5 * left_arm_length) / self.robot_arm_length
    ...
    def publish_joy(self, joy_data: dict) -> None:
        ...
        joy_msg.buttons = [int(btn[0]) for btn in buttons] + [int(btn[1]) for btn in buttons]
