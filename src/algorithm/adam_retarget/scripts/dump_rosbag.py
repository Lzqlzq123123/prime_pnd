...
def rosbag_iterator(rosbag_folder):
    ...
    for root, _dirs, files in os.walk(rosbag_folder):
        for file in files:
...
def process_bag_file(bag_file, output_folder):
    ...
    # e.g. ros2 launch adam_retarget dump_bone.launch.py bag:="rosbag/rosbag2_2025_03_17-11_26_37" dump_folder:="output"
    os.system(
...
def main():
    ...
    if not os.path.isdir(rosbag_folder):
        print(f"Error: ROS bag folder '{rosbag_folder}' not found or is not a directory.")
        return
