...
class AdamMinkBase(Node, ABC):
    ...
    def _load_parameters(self) -> tuple[str, str, bool]:
        ...
        if not adam_model_path:
            raise ValueError("Required parameter 'adam_model_path' is not set or is empty")
        if not adam_mink_cfg_path:
            raise ValueError("Required parameter 'adam_mink_cfg' is not set or is empty")
    ...
    def _load_model(self, adam_model_path: str) -> None:
        ...
        try:
            ...
            self.get_logger().fatal(f"Failed to load MuJoCo model from {adam_model_path}: {e}")
            raise
    ...
    def _load_config(self, adam_mink_cfg_path: str) -> None:
        ...
        try:
            with open(adam_mink_cfg_path, encoding="utf-8") as f:
    ...
    def _initialize_joint_mappings(self) -> None:
        ...
        self.all_joint = {
            name: i for i, name in enumerate(list(self.robot_motor_names.keys()) + list(ALL_FINGER))
        }
    ...
    def ik_thread_loop(self) -> None:
        ...
        while rclpy.ok():
            ...
            #     self.get_logger().warning(f"IK thread loop took {time.time() - start_time} seconds, which is longer than 25ms")
    ...
    def _initialize_mocap_data(self) -> MocapData:
        ...
        return {bone_name: (default_pos.copy(), default_rot.copy()) for bone_name in unique_bones}
    ...
    def _update_ik_targets(self) -> bool:
        ...
        for task, cfg in zip(self.tasks, self.adam_mink_cfg.ik_cfg, strict=True):
    ...
    def scale_mocap_data(self, data: MocapData) -> None:
        ...
        for body_name in data:
            if body_name not in self.adam_mink_cfg.human_scale_table:
                continue
            if body_name == "Hips":
                continue
            else:
        ...
        for body_name in human_data_local:
    ...
    def offset_mocap_data(self, data: MocapData) -> None:
        ...
        for mocap_data_name, (pos, rot) in data.items():
            if mocap_data_name in bone_name_to_cfg:
                ...
                rot_combined = quat_mul_single(rot, rot_b) if rot_b is not None else rot
            ...
            with mjv.launch_passive(
                ...
                while viewer.is_running():
                    try:
                        ...
                        self.get_logger().error(f"Error in Mujoco viewer loop: {e}", exc_info=True)
                        rate.sleep()
        except Exception as e:
