#!/usr/bin/env python3
"""Vive Tracker 3-based Adam robot inverse kinematics node using Mink solver."""
...
class AdamMinkViveTracker3Node(AdamMinkBase):
    """Vive Tracker 3-based Adam robot IK node with joystick control."""

    def __init__(self, node_name: str = "adam_mink_vive_tracker3") -> None:
        """Initialize the Vive Tracker 3 node."""
        super().__init__(node_name)
        self.calibrated = False
        qos = QoSProfile(durability=DurabilityPolicy.TRANSIENT_LOCAL, depth=1)
        self.sub_calibrated = self.create_subscription(
            Bool, "vive_mocap/calibrated", self.calibrated_callback, qos
        )
        ...
        self.get_logger().info("Adam Mink Vive Tracker 3 IK node initialized.")
...
def main(args=None) -> None:
    """Main entry point for the Vive Tracker 3 node."""
    rclpy.init(args=args)
    print(f"Starting Adam Mink Vive Tracker 3 IK node... {args}")

    node = AdamMinkViveTracker3Node()
