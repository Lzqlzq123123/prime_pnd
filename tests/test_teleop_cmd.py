...
def test_get_launch_command_by_stem_returns_none_if_missing(teleop_cmd, monkeypatch, tmp_path):
    monkeypatch.setattr(teleop_cmd, "_get_bringup_launch_dir", lambda: tmp_path)
    assert teleop_cmd._get_launch_command_by_stem("does-not-exist") is None


def test_get_launch_command_by_stem_returns_ros2_command(teleop_cmd, monkeypatch, tmp_path):
    _mk_launch_files(tmp_path, ["foo.launch.py"])
    monkeypatch.setattr(teleop_cmd, "_get_bringup_launch_dir", lambda: tmp_path)

    assert teleop_cmd._get_launch_command_by_stem("foo") == "ros2 launch bringup foo.launch.py"


def test_discover_launch_map_parses_expected_combinations(teleop_cmd, monkeypatch, tmp_path):
    _mk_launch_files(
        tmp_path,
        [
            "pinocchio-adam_sp-noitom.launch.py",
...
def test_get_valid_options_from_map_returns_discovered_values(teleop_cmd, monkeypatch, tmp_path):
    _mk_launch_files(
        tmp_path,
        [
            "pinocchio-adam_sp-noitom.launch.py",
...
def test_complete_algorithm_filters_by_ctx_params(teleop_cmd, monkeypatch, tmp_path):
    ...
    ctx = types.SimpleNamespace(params={"adam_type": "adam_u", "mocap_driver": "noitom"})
